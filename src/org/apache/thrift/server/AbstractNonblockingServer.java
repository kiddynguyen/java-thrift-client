...
public abstract class AbstractNonblockingServer extends TServer {
    ...
    /**
     * The maximum amount of memory we will allocate to client IO buffers at a time. Without this limit, the server will
     * gladly allocate client buffers right into an out of memory exception, rather than waiting.
     */
    ...
    /**
     * Perform an invocation. This method could behave several different ways - invoke immediately inline, queue for
     * separate execution, etc.
     * 
     * @return true if invocation was successfully requested, which is not a guarantee that invocation has completed.
     *         False if the request failed.
     */
    protected abstract boolean requestInvoke(FrameBuffer frameBuffer);

    /**
     * An abstract thread that handles selecting on a set of transports and {@link FrameBuffer FrameBuffers} associated
     * with selected keys corresponding to requests.
     */
    protected abstract class AbstractSelectThread extends Thread {
        ...
        /**
         * Add FrameBuffer to the list of select interest changes and wake up the selector if it's blocked. When the
         * select() call exits, it'll give the FrameBuffer a chance to change its interests.
         */
        ...
        /**
         * Check to see if there are any FrameBuffers that have switched their interest type from read to write or vice
         * versa.
         */
    ...
    /**
     * Class that implements a sort of state machine around the interaction with a client and an invoker. It manages
     * reading the frame size and frame data, getting it handed off as wrapped transports, and then the writing of
     * response data back to the client. In the process it manages flipping the read and write bits on the selection key
     * for its client.
     */
    public class FrameBuffer {
        ...
        /**
         * Give this FrameBuffer a chance to read. The selector loop should have received a read event for this
         * FrameBuffer.
         * 
        ...
        /**
         * After the processor has processed the invocation, whatever thread is managing invocations should call this
         * method on this FrameBuffer so we know it's time to start trying to write again. Also, if it turns out that
         * there actually isn't any data in the response buffer, we'll skip trying to write and instead go back to
         * reading.
         */
        ...
        /**
         * When this FrameBuffer needs to change its select interests and execution might not be in its select thread,
         * then this method will make sure the interest change gets done when the select thread wakes back up. When the
         * current thread is this FrameBuffer's select thread, then it just does the interest change immediately.
         */
