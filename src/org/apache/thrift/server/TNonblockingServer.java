 ...
 *
 * This server is inherently single-threaded. If you want a limited thread pool coupled with invocation-fairness, see
 * THsHaServer.
 *
 * To use this server, you MUST use a TFramedTransport at the outermost transport, otherwise this server will be unable
 * to determine when a whole method call has been read off the wire. Clients must also use TFramedTransport.
 */
public class TNonblockingServer extends AbstractNonblockingServer {
    ...
    protected class SelectAcceptThread extends AbstractSelectThread {
        ...
        /**
         * The work loop. Handles both selecting (all IO operations) and managing the selection preferences of all
        ...
        /**
         * Select and process IO events appropriately: If there are connections to be accepted, accept them. If there
         * are existing connections with data waiting to be read, read it, buffering until a whole frame has been read.
         * If there are any pending responses, buffer them until their target client is available, and then send the
         * data.
         */
        private void select() {
            try {
                // Wait for io events.
                // Block here until there are at least one event (new connection, readable, writable) occurs
                selector.select();
