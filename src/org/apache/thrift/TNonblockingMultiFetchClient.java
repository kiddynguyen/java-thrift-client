...
/**
 * This class uses a single thread to set up non-blocking sockets to a set of remote servers (hostname and port pairs),
 * and sends a same request to all these servers. It then fetches responses from servers.
 *
 * Parameters: int maxRecvBufBytesPerServer - an upper limit for receive buffer size per server (in byte). If a response
 * from a server exceeds this limit, the client will not allocate memory or read response data for it.
 *
 * int fetchTimeoutSeconds - time limit for fetching responses from all servers (in second). After the timeout, the
 * fetch job is stopped and available responses are returned.
 *
 * ByteBuffer requestBuf - request message that is sent to all servers.
 *
 * Output: Responses are stored in an array of ByteBuffers. Index of elements in this array corresponds to index of
 * servers in the server list. Content in a ByteBuffer may be in one of the following forms: 1. First 4 bytes form an
 * integer indicating length of following data, then followed by the data. 2. First 4 bytes form an integer indicating
 * length of following data, then followed by nothing - this happens when the response data size exceeds
 * maxRecvBufBytesPerServer, and the client will not read any response data. 3. No data in the ByteBuffer - this happens
 * when the server does not return any response within fetchTimeoutSeconds.
 *
 * In some special cases (no servers are given, fetchTimeoutSeconds less than or equal to 0, requestBuf is null), the
 * return is null.
 *
...
public class TNonblockingMultiFetchClient {
    ...
    /**
     * Private class that does real fetch job. Users are not allowed to directly use this class, as its run() function
     * may run forever.
     */
    private class MultiFetch implements Runnable {
         ...
         *
         * Server responses are stored in TNonblocingMultiFetchClient.recvBuf, and fetch statistics is in
         * TNonblockingMultiFetchClient.stats.
         *
         * Sanity check for parameters has been done in TNonblockingMultiFetchClient before calling this function.
         */
        public void run() {
                ...
                while (it.hasNext()) {
                    ...
                    if (selKey.isValid() && selKey.isConnectable()) {
                        ...
                        try {
                            ...
                            stats.incNumConnectErrorServers();
                            String err = String.format("socket %d connects to server %s " + "error: %s", index, servers.get(index).toString(),
                                    e.toString());
                            LOGGER.error(err);
                        }
                    }
                    ...
                    if (selKey.isValid() && selKey.isWritable()) {
                        if (sendBuf[index].hasRemaining()) {
                            ...
                            try {
                                ...
                                String err = String.format("socket %d writes to server %s " + "error: %s", index, servers.get(index).toString(),
                                        e.toString());
                                LOGGER.error(err);
                            }
                        }
                    ...
                    if (selKey.isValid() && selKey.isReadable()) {
                        ...
                        try {
                            ...
                            if (bytesRead > 0) {
                                ...
                                if (!hasReadFrameSize[index] && recvBuf[index].remaining() == 0) {
                                    ...
                                    if (frameSize[index] <= 0) {
                                        ...
                                        String err = String
                                                .format("Read an invalid frame size %d" + " from %s. Does the server use TFramedTransport? ",
                                    ...
                                    if (frameSize[index] + 4 > maxRecvBufBytesPerServer) {
                                        ...
                                        String err = String.format("Read frame size %d from %s," + " total buffer size would exceed limit %d",
                                                frameSize[index], servers.get(index).toString(), maxRecvBufBytesPerServer);
                                        LOGGER.error(err);
                                        sChannel.close();
                                        continue;
                                    }
                            ...
                            String err = String.format("socket %d reads from server %s " + "error: %s", index, servers.get(index).toString(),
                                    e.toString());
                            LOGGER.error(err);
                        }
                    }
