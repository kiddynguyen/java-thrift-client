...
/**
 * A superclass for SASL client/server thrift transports. A subclass need only implement the <code>open</open> method.
 */
abstract class TSaslTransport extends TTransport {
    ...
    /**
     * Whether or not we should wrap/unwrap reads/writes. Determined by whether or not a QOP is negotiated during the
     * SASL handshake.
     */
    ...
    /**
     * Create a TSaslTransport. It's assumed that setSaslServer will be called later to initialize the SASL endpoint
     ...
     *
     * @param saslClient
     *            The <code>SaslClient</code> which this transport will use for SASL negotiation.
     * @param underlyingTransport
     *            The thrift transport which this transport is wrapping.
     */
     ...
     * @throws TTransportException
     *             Thrown if there is a failure reading from the underlying transport, or if a status code of BAD or
    ...
    /**
     * Send a Thrift SASL message with the given status (usually BAD or ERROR) and string message, and then throw a
     * TTransportException with the given message.
     *
     * @param status
     ...
     * @return always throws TTransportException but declares return type to allow throw sendAndThrowMessage(...) to
     *         inform compiler control flow
     */
    ...
    /**
     * Implemented by subclasses to start the Thrift SASL handshake process. When this method completes, the
     * <code>SaslParticipant</code> in this class is assumed to be initialized.
     *
     * @throws TTransportException
    ...
    /**
     * Opens the underlying transport if it's not already open and then performs SASL negotiation. If a QOP is
     * negotiated during this SASL handshake, it used for all communication on this transport after this call is
     * complete.
     */
    @Override
    public void open() throws TTransportException {
        /*
         * readSaslHeader is used to tag whether the SASL header has been read properly. If there is a problem in
         * reading the header, there might not be any data in the stream, possibly a TCP health check from load
         * balancer.
         */
        ...
        try {
            ...
            /*
             * If there is no-data or no-sasl header in the stream, throw a different type of exception so we can handle
    ...
    /**
     * Used to abstract over the <code>SaslServer</code> and <code>SaslClient</code> classes, which share a lot of their
     * interface, but unfortunately don't share a common superclass.
     */
