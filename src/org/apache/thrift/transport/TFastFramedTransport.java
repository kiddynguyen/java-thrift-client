...
/**
 * This transport is wire compatible with {@link TFramedTransport}, but makes use of reusable, expanding read and write
 * buffers in order to avoid allocating new byte[]s all the time. Since the buffers only expand, you should probably
 * only use this transport if your messages are not too variably large, unless the persistent memory cost is not an
 * issue.
 * 
...
public class TFastFramedTransport extends TTransport {
    ...
    /**
     * Create a new {@link TFastFramedTransport}. Use the defaults for initial buffer size and max frame length.
     * 
     * @param underlying
     *            Transport that real reads and writes will go through to.
     */
    ...
    /**
     * Create a new {@link TFastFramedTransport}. Use the specified initial buffer capacity and the default max frame
     * length.
     * 
     * @param underlying
     *            Transport that real reads and writes will go through to.
     * @param initialBufferCapacity
     *            The initial size of the read and write buffers. In practice, it's not critical to set this unless you
     *            know in advance that your messages are going to be very large.
     */
     ...
     * @param initialBufferCapacity
     *            The initial size of the read and write buffers. In practice, it's not critical to set this unless you
     *            know in advance that your messages are going to be very large. (You can pass
     *            TFramedTransportWithReusableBuffer.DEFAULT_BUF_CAPACITY if you're only using this constructor because
     *            you want to set the maxLength.)
     * @param maxLength
     *            The max frame size you are willing to read. You can use this parameter to limit how much memory can be
     *            allocated.
     */
