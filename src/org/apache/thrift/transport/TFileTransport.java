...
/**
 * FileTransport implementation of the TTransport interface. Currently this is a straightforward port of the cpp
 * implementation
 * 
 * It may make better sense to provide a basic stream access on top of the framed file format The FileTransport can then
 * be a user of this framed file format with some additional logic for chunking.
 *
...
public class TFileTransport extends TTransport {
    ...
    public static enum TailPolicy {
         ...
         *
         * @param timeout
         *            sleep time for this particular policy
         * @param retries
         *            number of retries
         */
     ...
     *
     * @return true if recovery could be performed and we can read more data false is returned only when nothing more
     *         can be read
     */
    ...
    private boolean readEvent() throws TTransportException {
        ...
        retry: do {
            ...
            if (esize > cs.getRemaining()) {
                ...
                /*
                 * if(performRecovery()) { esize=0; } else { return false; }
                 */
            }
        } while (esize == 0);
    ...
    /**
     * Diverging from the cpp model and sticking to the TSocket model Files are not opened in ctor - but in explicit
     * open call
     */
     ...
     *
     * @param inputFile
     *            open TSeekableFile to read/write from
     * @param readOnly
     *            Whether this is a read-only transport
     */
