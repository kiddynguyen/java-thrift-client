...
/**
 * TCompactProtocol2 is the Java implementation of the compact protocol specified in THRIFT-110. The fundamental
 * approach to reducing the overhead of structures is a) use variable-length integers all over the place and b) make use
 * of unused bits wherever possible. Your savings will obviously vary based on the specific makeup of your structs, but
 * in general, the more fields, nested structures, short strings and collections, and low-value i32 and i64 fields you
 * have, the more benefit you'll see.
 */
public class TCompactProtocol extends TProtocol {
    ...
    /**
     * Write a struct begin. This doesn't actually put anything on the wire. We use it as an opportunity to put special
     * placeholder markers on the field stack so we can get the field id deltas correct.
     */
    ...
    /**
     * Write a struct end. This doesn't actually put anything on the wire. We use this as an opportunity to pop the last
     * field from the current struct off of the field stack.
     */
    ...
    /**
     * Write a field header containing the field id and field type. If the difference between the current field id and
     * the last one is small (< 15), then the field id will be encoded in the 4 MSB as a delta. Otherwise, the field id
     * will follow the type header as a zigzag varint.
     */
    ...
    /**
     * The workhorse of writeFieldBegin. It has the option of doing a 'type override' of the type header. This is used
     * specifically in the boolean field case.
     */
    ...
    /**
     * Write a map header. If the map is empty, omit the key and value type headers, as we don't need any additional
     * information to skip it.
     */
    ...
    /**
     * Write a boolean value. Potentially, this could be a boolean field, in which case the field header info isn't
     * written yet. If so, decide what the right type header is for the value and then write the field header.
    ...
    /**
     * Abstract method for writing the start of lists and sets. List and sets on the wire differ only by the type
     * indicator.
     */
    ...
    /**
     * Writes a byte without any possibility of all that field header nonsense. Used internally by other writing methods
     * that know they need to write a byte.
     */
    ...
    /**
     * Read a struct begin. There's nothing on the wire for this, but it is our opportunity to push a new struct begin
     * marker onto the field stack.
     */
    ...
    /**
     * Read a map header off the wire. If the size is zero, skip reading the key and value type. This means that
     * 0-length maps will yield TMaps without the "correct" types.
     */
    ...
    /**
     * Read a list header off the wire. If the list size is 0-14, the size will be packed into the element type header.
     * If it's a longer list, the 4 MSB of the element type header will be 0xF, and a varint will follow with the true
     * size.
     */
    ...
    /**
     * Read a set header off the wire. If the set size is 0-14, the size will be packed into the element type header. If
     * it's a longer set, the 4 MSB of the element type header will be 0xF, and a varint will follow with the true size.
     */
    ...
    /**
     * Read a boolean off the wire. If this is a boolean field, the value should already have been read during
     * readFieldBegin, so we'll just consume the pre-stored value. Otherwise, read a byte.
     */
    ...
    /**
     * Read an i32 from the wire as a varint. The MSB of each byte is set if there is another byte to follow. This can
     * read up to 5 bytes.
     */
    ...
    /**
     * Read an i64 from the wire as a proper varint. The MSB of each byte is set if there is another byte to follow.
     * This can read up to 10 bytes.
     */
    ...
    /**
     * Note that it's important that the mask bytes are long literals, otherwise they'll default to ints, and when you
     * shift an int left 56 bits, you just get a messed up int.
     */
    private long bytesToLong(byte[] bytes) {
        return ((bytes[7] & 0xffL) << 56) | ((bytes[6] & 0xffL) << 48) | ((bytes[5] & 0xffL) << 40) | ((bytes[4] & 0xffL) << 32)
                | ((bytes[3] & 0xffL) << 24) | ((bytes[2] & 0xffL) << 16) | ((bytes[1] & 0xffL) << 8) | ((bytes[0] & 0xffL));
    }
